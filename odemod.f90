...
CONTAINS 
  ...
  USE ode_path
  USE globalvariables, only: nf, freqs, nind2
  IMPLICIT NONE
  ...
  do nstp=1,MAXSTP+1
     write(*,*) 'step=', nstp,'x=',x
     if (mod(nstp,10)==0.and.(.false.)) then
        do i=1,size(y)
           write(456,*) y(i)
        end do
     ...
     do i=1,size(y)
        yscal(i)=max(abs(y(i))+abs(h*dydx(i)),1e-1_sp)
        !yscal(i)=abs(y(i))
     end do
    ...
    CONTAINS
      ...
      SUBROUTINE save_a_step
        ...
        xp(kount)=x
        yp(:,kount)=y(:)
        !yp(nf+freqs+1,kount)=dydx(nf+freqs+1)
        xsav=x
