...
CONTAINS
  ...
  SUBROUTINE midinf(funk,aa,bb,s,n,flag_stu,flag_sd,freq,fval,site)
    ...
    a=1.0_sp/bb
    if (n == 1) then
       s=(b-a)*sum(func( (/0.5_sp*(a+b)/),flag_stu,flag_sd,fval,freq,site ))
    else
       it=3**(n-2)
       del=(b-a)/(3.0_sp*it)
       x(1:2*it-1:2)=arth(a+0.5_sp*del,3.0_sp*del,it)
       x(2:2*it:2)=x(1:2*it-1:2)+2.0_sp*del
       s=s/3.0_sp+del*sum(func(x,flag_stu,flag_sd,fval,freq,site))
    end if
  CONTAINS
    !BL
    FUNCTION func(x,flag_stu,flag_sd,fval,freq,site)
      ...
      INTEGER, INTENT(IN) :: flag_stu, flag_sd, freq,site
      REAL(SP),INTENT(IN) :: fval
      func=funk(1.0_sp/x,flag_stu,flag_sd,fval,freq,site)/x**2
    END FUNCTION func
  END SUBROUTINE midinf
  
  SUBROUTINE midpnt(func,a,b,s,n,flag_stu,flag_sd,freq,fval,site)
